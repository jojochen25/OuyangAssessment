...
num3_df <- data |>
  dplyr::select(Month, Year, firstAuthorF) |>
  mutate(date = paste(Month, Year, sep="-")) |>
  mutate(date = as.Date(paste(Year, Month, "01", sep = "-"))) |>
  mutate(firstAuthorRem=gsub("(?<=\\s)[A-Z](\\.|\\s)?\\b|\\s\\w+$", "", firstAuthorF, perl = TRUE)) |>
  select(firstAuthorRem, date) |> # did not select firstAuthorF, 
...
new_firstName<-data |>
  ...
  count(firstAuthorRem) |>
  filter(n>30 & !is.na(firstAuthorRem)) |>
  dplyr::select(date, firstAuhorRem)
...
df_2 <- data |>
  dplyr::select(Month, Year, numAuthor) |>
  group_by(Year, Month) |>
  count(numAuthor > 2) |>
  mutate(`Month-Year` = paste(Month, Year, sep="-")) # make a new month-year variable
...
# # ifelse(n()>2, mean(diff_month, na.rm=TRUE), NaN)
# dates |>
#   filter(firstAuthorF=="Shantel") |>
#   arrange(date) |>
#   mutate(time_diff = date - lag(date)) |>
#   mutate(diff_month = as.numeric(round(time_diff/30.44))) |> # round to nearest month
#   summarize(ave_num_months = ifelse(n()>2, mean(diff_month, na.rm=TRUE), NaN),
#             ave_num_months = ifelse(is.nan(ave_num_months), -1, ave_num_months)) # if one pub, replace NaN withave_num_months=-1
